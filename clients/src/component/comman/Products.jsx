...
import { addWatchList, getWatchList, removeWatchList } from "../../service/watchList";
...
const Products = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const ids = watchListRes?.watchListItems?.map((item) => item.productId._id) || [];
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {products.map((product) => (
            <div key={product._id} className="bg-white p-4 rounded-md shadow-sm relative">
              <div className="h-60 bg-gray-200 mb-4 rounded-md overflow-hidden">
                {product.image ? (
                  ...
                  <p className="text-gray-400 text-sm text-center mt-16">No Image</p>
                )}
              </div>
              ...
              <h3 className="font-medium text-gray-700 truncate">{product.productTitle}</h3>
